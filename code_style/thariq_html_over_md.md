...
## Strongly recommended to read before continuing
- `~/research/docs/code_style/other/frontend/ryolu_design_frontend.md` — designing in layers: sequence the reveal of complexity, don't dump everything at once. Lead with the action layer, let reference collapse underneath.
- `~/research/docs/code_style/other/frontend/frontend_skill.md` — frontend aesthetics guidelines: commit to a bold aesthetic direction, avoid generic AI dark-mode defaults, be intentional about typography/color/spatial composition.
...
--
Quality Guardrails for Generated HTML
LLMs produce HTML with predictable failure modes. Prevent by construction, validate after.

Defensive CSS (always include):
- `* { box-sizing: border-box; }` — prevents padding-induced overflow
- `table { table-layout: fixed; width: 100%; }` + `td, th { overflow-wrap: break-word; }` — tables can't blow out containers
- `pre, code { overflow-x: auto; white-space: pre-wrap; }` — code blocks wrap instead of overflowing
- `img, svg { max-width: 100%; height: auto; }` — media stays in bounds
- Container elements: `overflow: hidden` or `overflow: auto`

Contrast (WCAG AA minimum):
- 4.5:1 for normal text, 3:1 for large text (≥18px bold or ≥24px)
- Light code blocks with dark text, always. Dark backgrounds with syntax highlighting are the #1 readability failure.
- Document contrast ratios in a CSS comment block for each color pair
- Formula: `(L1 + 0.05) / (L2 + 0.05)` where L is relative luminance

Validation tools:
- `npx htmlhint <file>` — structural HTML lint
- `stylelint` with `stylelint-a11y` plugin — CSS lint + accessibility checks
- Inline self-check: add a `<script>` that walks the DOM, computes contrast ratios, flags overflow. See `/tmp/html-quality-template.html` for a reference implementation.
- `@chenglou/pretext` — pure-JS text measurement without DOM reflow, useful if building layout validation

Template: `~/research/docs/code_style/other/frontend/html-quality-template.html` — start from this. Includes defensive CSS, WCAG-compliant palette, and inline validator that flags contrast failures, container overflow, table-layout risks, and SVG text overflow (text exceeding its containing rect, with suggested widths).
