...
- **Comments exist for the unintuitive**: If code needs a comment to explain what it does, fix the code (better names, clearer control flow). Comments earn their place only when explaining why something non-obvious is correct — perf workarounds, algorithm sources, external system constraints, "this looks wrong but is intentional because X." Assertions can serve as documentation too. *(Tiger Style, adapted)* — "Use a blatantly true assertion instead of a comment as stronger documentation where the assertion condition is critical and surprising."
...
- **Parse at boundaries, trust internally**: All validation and external-data normalization lives at entry points (`__post_init__`, API handlers, CLI parsing). Once data passes the boundary, internal code trusts it. Don't re-validate deep in the call stack. *(My Notes + courier practice)*
- **Ask permission, not forgiveness**: If a precondition can be queried directly (file exists, key in dict, connection alive), check it explicitly and fail loudly rather than calling, failing, and parsing the failure. Exceptions are for things you can't predict, not things you can look up. Caveat: for concurrent/external state (files, network), TOCTOU makes this unreliable — wrap at the boundary instead. *(My Notes)*
- **Sum types for known failure modes**: When an operation has expected failure cases (network timeout, rate limit, malformed response), model them as a discriminated union (`Ok[T] | Err[E] | Pending[R]`) and match exhaustively. This makes failure modes part of the type signature and forces callers to handle each case visibly. Strictly better than `(result, error)` tuples because the type checker can enforce exhaustiveness. *(courier retry.py pattern)*
- **try/except only wraps external calls**: Use at I/O boundaries (HTTP, file, subprocess, third-party libs) and immediately translate into domain types. Never let library exceptions propagate through internal code — catch once, convert to your sum type or raise a domain-specific error. *(My Notes)*
- **Assertions for programmer bugs**: `assert` for invariants that should never fail if code is correct. If it fires, it's a bug in your code, not a runtime condition. Use for unreachable branches, precondition checks on trusted data, and documenting assumptions that the type system can't express. *(Tiger Style)*
- **Crash loudly for boundary violations**: `raise` with a specific error when the world is wrong (missing config, invalid input, infrastructure failure). No recovery, no fallbacks. The stack trace is the error message. *(My Notes)*
- **Provide explicit variants**: Offer `get_user()`, `get_user_or_none()`, `get_user_or_create()` so callers choose their control flow rather than being forced into try/except for common cases. *(Casey Code Reuse)*
- **The only legitimate internal "handling"**: When failure is inherent to the operation (streaming partial data, incremental parsing) and the caller has a real decision to make (retry vs abort, skip vs escalate). Capture the error as data, don't raise. *(courier sglang.py streaming pattern)*
...
- **Document the why, only when non-obvious**: Leave comments explaining reasoning when bucking conventions or doing something unintuitive. If you need a comment to explain what, refactor instead. *(Tiger Style, adapted)*
