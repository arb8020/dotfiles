...
## Consolidate mutation surface

When a stateful object exists, all mutations to its state should go through
a small number of methods — not be spread across callers.

Bad:

```python
# Three callers each reaching into the bag
state.engines[model] = engine
state.concurrency_limiters[model] = trio.CapacityLimiter(engine.max_concurrent)
state.readiness.register(model)
```

Good:

```python
# One method enforces the coordinated update
def register_engine(self, engine: EngineEndpoint) -> None:
    self.engines[engine.model] = engine
    self.concurrency_limiters[engine.model] = trio.CapacityLimiter(engine.max_concurrent)
    self.readiness.register(engine.model)
```

If forgetting one of the coordinated updates breaks things silently, that
coordination belongs behind a method, not scattered across call sites.
